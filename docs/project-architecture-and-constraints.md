...
> 최종 확인 기준: 2026-07-29, `main` 브랜치 `5c034ae`
>
...
1. 사용자가 사진 접근 권한을 허용한다.
2. 앱이 사진 보관함의 스크린샷 스마트 앨범을 읽는다.
3. iPhone에 원본이 실제로 내려받아져 있는 사진만 가져온다.
4. Apple Vision으로 OCR과 이미지 분류를 기기 안에서 수행한다.
5. OCR 결과에서 이메일, 전화번호, 주민등록번호, 카드번호 패턴을 가린다.
6. 가린 텍스트와 이미지 라벨을 CapLog 백엔드로 보낸다.
7. 백엔드는 서버 환경변수에 있는 OpenAI API 키로 OpenAI에 요청한다.
8. GPT가 카드 제목, 요약, 카테고리, 태그, 날짜, 장소 등의 구조를 반환한다.
9. GPT 호출 또는 결과 파싱에 실패해도 OCR 기반 대체 카드를 만든다.
10. 구조화된 카드 내용은 서버 DB에 저장한다.
11. 카드 이미지 사본은 iPhone 앱 내부 저장소에만 저장한다.
12. 생성 결과와 실패 건수는 마이페이지의 카드 생성 상태에서 확인한다.
...
로컬 전용 카드, 로컬 이미지, 최근 검색, 최근 본 카드, 앱 내부 알림 읽음 상태가 사용자 계정별 저장 공간으로 분리되어 있지 않다. 로그아웃은 토큰과 프로필 캐시를 지우지만 이 데이터 전체를 자동 삭제하지 않는다.
