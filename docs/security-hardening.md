...
- 위치 권한이 이미 허용된 경우에만 기기의 현재 위치를 한 번 조회하며 위치를 지속 추적하지 않습니다.
- 사용자 좌표를 인증된 근처 추천 API에 전달하고 현재 사용자의 카드만 거리순으로 조회합니다.
- 장소 카드 저장 시 `장소명`·`place_name`과 `주소`·`address` 필드를 인식해 좌표 변환을 자동 실행합니다.
- 홈 추천 영역에 근처 카드와 거리를 표시하고, 권한 없음·위치 조회 실패·서버 오류 시 기존 최신 카드 추천을 유지합니다.
- 서버는 위도·경도의 유효 범위를 검사하고 반경과 결과 개수를 제한합니다.
