...
- 원본 스크린샷은 서버·DB·Google Vision으로 전송하지 않습니다.
- Apple Vision의 OCR과 이미지 분류를 iPhone 안에서 실행합니다.
- GPT에는 원본 이미지가 아닌 OCR 텍스트만 전달합니다.
- GPT 전송 전 이메일, 전화번호, 주민등록번호, 카드번호 형식을 자동으로 가립니다.
- 카드 제목·요약·카테고리·태그·상세 필드만 서버 DB에 저장합니다.
- 카드 이미지는 앱의 `Application Support/CardImages`에 파일 보호를 적용해 저장합니다.
- 카드 이미지 디렉터리와 파일은 iCloud 및 기기 백업 대상에서 제외합니다.
- 서버 이미지 업로드·조회 API와 Google Vision 중계 API를 제거했습니다.

아래의 Google Vision·서버 업로드 관련 내용은 이전 보안 강화 이력이며 현재 동작에는 적용되지 않습니다. 기존 운영 서버의 `screenshot_file` 레코드나 `data/uploads` 파일은 이번 코드 변경에서 자동 삭제하지 않으므로, 존재한다면 백업·보존 정책을 확인한 뒤 별도 관리 작업으로 제거해야 합니다.
...
### 20. 카드 수정·삭제 서버 영구 반영

- iOS 카드 수정과 삭제를 인증된 `PUT /api/cards/{id}`, `DELETE /api/cards/{id}` API에 연결했습니다.
- 서버는 카드 ID와 로그인 사용자를 함께 조회해 다른 사용자의 카드를 수정하거나 삭제할 수 없도록 했습니다.
- 제목, 요약, 대·소분류, 태그, 상세 필드, 이미지와 생성·수정 시간을 DB에 저장해 앱 재실행 후에도 편집 결과가 유지됩니다.
- 장소명이나 주소가 바뀌면 기존 좌표를 폐기하고 새 위치를 다시 변환합니다.
- 서버 저장에 실패한 카드는 앱 화면에서 성공으로 표시하거나 먼저 삭제하지 않습니다.
- 서버에 전송되지 못했던 로컬 전용 카드는 기존처럼 기기 안에서 수정·삭제할 수 있습니다.

### 21. 실제 계정 기반 친구 관리

- 친구 목록 화면에서 개발용 샘플 사용자를 제거하고 인증된 계정의 서버 친구 목록만 표시합니다.
- 친구 ID로 추가하면 서버가 두 사용자 방향의 관계를 함께 저장해 상대방의 친구 목록에도 반영됩니다.
- 예전 데이터에 한쪽 관계만 남아 있으면 누락된 반대 방향을 자동으로 복구합니다.
- 친구 삭제는 두 사용자 방향의 관계를 함께 삭제하며, 존재하지 않는 관계를 성공으로 처리하지 않습니다.
- iOS 앱은 서버의 추가·삭제 응답을 기다린 후에만 화면을 변경하고, 실패하면 빈 목록·재시도·오류 안내를 표시합니다.

### 22. 인증 필터 서버 실행 안정화

- 생성자가 둘인 요청 제한 필터에서 실제 운영 생성자를 명확히 지정해 Spring Boot 서버가 정상 시작되도록 수정했습니다.
- JWT 인증과 요청 제한 필터가 서블릿 컨테이너에 자동 등록되지 않도록 막고 Spring Security 체인에서만 한 번 실행되도록 했습니다.
- 인증 전·후 필터 순서를 유지해 사용자별 요청 제한이 IP 제한으로 잘못 계산되지 않도록 했습니다.
...
- `PUT /api/cards/{id}`
  - 로그인 사용자가 소유한 카드만 수정할 수 있습니다.
- `DELETE /api/cards/{id}`
  - 로그인 사용자가 소유한 카드만 삭제할 수 있습니다.
- `screenshot_file` 테이블
  - `storage_key`, `content_type`, `size_bytes` 컬럼을 사용합니다.

## 검증 결과

- 백엔드 Gradle 테스트 26개 통과
